...
def k_contrast_color(k_color, HSV_list):
  ...
  # saturation: 40-80, min_step: 0.1(5)
  # intensive : 50-100, min_step: 0.1(6)
  # max_color = 72*6*6 = 2592

  H_min_step = 10
  H_max_num = 360 // H_min_step
  S_min_step = 10
  S_max_num = math.floor(40 / S_min_step) + 1
  I_min_step = 10
  I_max_num = math.floor(50 / I_min_step) + 1
    ...
    if k_color <= H_max_num*S_max_num:
      if k_color <= H_max_num: 
        ...
        for i in range(k_color):
          ...
          tmp_I = 1.0 - Ii * I_step
          HSV_list.append([tmp_H, tmp_S, tmp_I])
    ...
    for i in range(grid_len):
      for j in range(grid_wid):
        ...
        hsv_color = (hsv_colors[j*grid_len+i][0], 
                      hsv_colors[j*grid_len+i][1]*100,
                      hsv_colors[j*grid_len+i][2]*100,
                      100)
