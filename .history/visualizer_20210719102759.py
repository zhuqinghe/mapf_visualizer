...
class Visualizer(object):
  ...
  def display(self):
    ...
    for i in range(self.agent_num): # render starts and ends
      start_x = self.Agents[i].path[0][1]
      start_y = self.Agents[i].path[0][0]
      end_x = self.Agents[i].path[-1][1]
      end_y = self.Agents[i].path[-1][0]
      gfxdraw.rectangle(starts, 
                        pygame.Rect(start_x-grid_size, top, width, height), 
                        color)
      gfxdraw.box(starts, 
