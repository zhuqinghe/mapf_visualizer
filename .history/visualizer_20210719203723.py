  ...
  def display(self):
    ...
    while 1:
        ...
        for i in range(self.agent_num): # render agent
          ...
          if self.time < cur_agent.path_len-1: # not arrive
            ...
            pix_x = round(grid_pos[1]*grid_size+
                          grid_shift_x*(self.frame_count%self.fps)/self.fps*grid_size)
            pix_y = round(grid_pos[0]*grid_size+
                          grid_shift_y*(self.frame_count%self.fps)/self.fps*grid_size)
          else: # arrive
      ...
      pygame.display.set_caption("MAPF Visualizer    FPS: " + 
                                ...
                                "    Timestep: " + 
                                "{:2.2f}".format(self.time+(self.frame_count%self.fps)/self.fps)) 
