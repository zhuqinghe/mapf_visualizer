...
class Visualizer(object):
  def __init__(self, map_file, paths_file, grid_size=30, fps=30, show_start=1, save_gif=1):
    ...
    for i in range(self.agent_num):
      ...
      if self.max_time < self.Agents[i].path_len:
        self.max_time = self.Agents[i].path_len
      ...
      if self.finish: 
        ...
        if self.save_gif:
          if self.frame_count < 10:
            pygame.image.save(screen, f"screenshot0{self.frame_count}.png")
          else:
            pygame.image.save(screen, f"screenshot{self.frame_count}.png")
